...
| FR-09 | The system shall allow a user to mark an appointment as completed or cancelled and to reopen a completed appointment when permitted. Reopening is permitted only to the appointment owner within 7 days of completion, must be recorded in the audit log with reason, and may be restricted by business policy (e.g., after finalization windows). | UC-12     | BR-14                               |
